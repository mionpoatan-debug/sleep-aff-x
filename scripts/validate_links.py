...
class RakutenLinkParser(HTMLParser):
    ...
    def handle_starttag(self, tag: str, attrs: list[tuple[str, str | None]]) -> None:
        ...
        key = attr_map.get("data-rakuten-key", "").strip()
        if not key:
            return
...
def load_csv_mapping(csv_path: Path) -> tuple[dict[str, str], list[str]]:
    ...
    with csv_path.open("r", encoding="utf-8", newline="") as f:
        ...
        expected_columns = {"key", "rakuten_url"}
        ...
        if fieldnames != expected_columns:
            errors.append(
                f"{csv_path}: CSV header must be key,rakuten_url (got: {','.join(reader.fieldnames)})"
            )
            return mapping, errors
